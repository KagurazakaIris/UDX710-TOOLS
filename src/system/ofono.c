...
int ofono_reload(void) {
  ...
  /* 重新初始化 D-Bus */
  if (!ofono_init()) {
    printf("[Reload] ofono_init 失败\n");
    return 0;
  }

  /* 重新启动数据监听 */
  if (ofono_start_data_monitor() != 0) {
    printf("[Reload] ofono_start_data_monitor 失败\n");
    return 0;
  }

  printf("[Reload] ofono 重新加载完成\n");
  return 1;
}
